...
public static unsafe class CUE4ParseNatives
{
    ...
    static CUE4ParseNatives()
    {
        if (!NativeLibrary.TryLoad(
                ...
                out var handle))
        {
            var fileName = OperatingSystem.IsWindows() ? $"{LibraryName}.dll" : OperatingSystem.IsMacOS() ? $"lib{LibraryName}.dylib" : $"lib{LibraryName}.so";
            if (!NativeLibrary.TryLoad(Path.Combine(AppContext.BaseDirectory, fileName), out handle))
            {
