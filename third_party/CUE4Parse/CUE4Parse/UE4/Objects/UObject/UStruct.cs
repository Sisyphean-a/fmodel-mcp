...
{
    [JsonIgnore]
    public string ScriptReadState { get; private set; } = "notRead";
    [JsonIgnore]
    public int SerializedScriptSize { get; private set; }
    [JsonIgnore]
    public int ParsedScriptSize { get; private set; }
    [JsonIgnore]
    public string? ScriptFailure { get; private set; }
    ...
    public override void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        if (Ar.Owner!.Provider?.ReadScriptData == true && Ar.Game >= GAME_UE4_0 && serializedScriptSize > 0)
        {
            ...
            try
            {
                scriptBytes = Ar.ReadBytes(serializedScriptSize);
            }
            catch (Exception e)
            {
                ScriptReadState = "failed";
                ScriptFailure = $"{e.GetType().Name}: {e.Message}; position={Ar.Position}";
                return;
            }
            using var kismetAr = new FKismetArchive(Name, scriptBytes, Ar.Owner, Ar.Versions);
            ...
            try
            {
                while (kismetAr.Position < kismetAr.Length)
                {
                    ...
                    lastSuccessfulPosition = kismetAr.Position;
                    ParsedScriptSize = checked((int)Math.Min(lastSuccessfulPosition, int.MaxValue));
                }

                ScriptReadState = "complete";
            }
            catch (Exception e)
            {
                ParsedScriptSize = checked((int)Math.Min(lastSuccessfulPosition, int.MaxValue));
                ScriptReadState = tempCode.Count > 0 ? "partial" : "failed";
                ScriptFailure = $"{e.GetType().Name}: {e.Message}; position={kismetAr.Position}; index={kismetAr.Index}";
        ...
        else
        {
            ...
            ScriptReadState = Ar.Owner?.Provider?.ReadScriptData == true && Ar.Game >= GAME_UE4_0 ? "empty" : "notRead";
        }
    }
