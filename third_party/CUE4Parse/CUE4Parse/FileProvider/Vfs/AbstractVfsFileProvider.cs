...
{
    public sealed record VfsMountFailureDiagnostic(
        string ArchivePath,
    ...
    public abstract class AbstractVfsFileProvider : AbstractFileProvider, IVfsFileProvider
    {
        ...
        public event EventHandler<VfsMountFailureDiagnostic>? VfsMountFailed;

        private void ReportMountFailure(IAesVfsReader reader, string phase, Exception exception)
            => ReportMountFailure(reader.Path, reader.EncryptionKeyGuid.ToString(), phase, exception);

        private void ReportMountFailure(string archivePath, string encryptionKeyGuid, string phase, Exception exception)
        {
            VfsMountFailed?.Invoke(this, new VfsMountFailureDiagnostic(
                archivePath,
                encryptionKeyGuid,
                phase,
        ...
        public void RegisterVfs(FArchive archive, Stream? stream, Func<string, FArchive>? openContainerStreamFunc = null)
        {
            ...
            {
                ReportMountFailure(archive.Name, string.Empty, "register", e);
        ...
        public void RegisterRandomAccessVfs(FArchive pakOrUtocArchive, FArchive? utocArchive, Func<string, FArchive>? openContainerStreamFunc = null)
        {
            ...
            {
                ReportMountFailure(pakOrUtocArchive.Name, string.Empty, "register", e);
                Log.Warning(e.ToString());
            }
        }
        ...
        {
            if (reader.IsEncrypted && CustomEncryption == null)
            {
                ReportMountFailure(reader, "mount-precheck", new InvalidOperationException("encrypted archive requires an AES key"));
                return;
            }
            if (!reader.HasDirectoryIndex)
            {
                ReportMountFailure(reader, "mount-precheck", new InvalidDataException("archive directory index is unavailable"));
                return;
            }
            ...
            {
                ReportMountFailure(reader, "mount", e);
            }
            catch (Exception e)
            {
                ReportMountFailure(reader, "mount", e);
        ...
        public async Task<int> SubmitKeysAsync(IEnumerable<KeyValuePair<FGuid, FAesKey>> keys)
        {
            ...
            foreach (var (guid, key) in submittedKeys)
            {
                foreach (var reader in readers.Where(it => it.EncryptionKeyGuid == guid))
                {
                    ...
                    if (!reader.HasDirectoryIndex)
                    {
                        ReportMountFailure(reader, "submitKey-precheck", new InvalidDataException("archive directory index is unavailable"));
                        continue;
                    }
                    ...
                    tasks.AddLast(Task.Run(() =>
                    {
                        ...
                        {
                            ReportMountFailure(reader, "submitKey", e);
                        }
                        catch (Exception e)
                        {
                            ReportMountFailure(reader, "submitKey", e);
