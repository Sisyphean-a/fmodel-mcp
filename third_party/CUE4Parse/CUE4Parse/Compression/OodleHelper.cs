...
public static class OodleHelper
{
    ...
    public static Task InitializeAsync(string? path = null, CancellationToken cancellationToken = default)
    {
        if (Instance is not null) return Task.CompletedTask;
        ...
        if (!string.IsNullOrWhiteSpace(path) && File.Exists(path))
        {
            ...
            Initialize(new Oodle(path));
            return Task.CompletedTask;
        }

        Log.Warning("Oodle decompression unavailable: no approved local Native library");
        return Task.CompletedTask;
    }
        ...
        FArchive? reader = null)
    {
        ...
        {
            throw new OodleException(reader, $"Oodle decompression returned {decodedSize} bytes, expected {uncompressedSize}");
        }
    }
